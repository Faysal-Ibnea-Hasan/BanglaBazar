<form action="{{ route('sub_categories.update') }}" method="POST" class="row g-3">
    ...
    <div class="col-md-6">
        ...
        <select name="category_id" class="form-select status" id="inputGroupSelect01">
            ...
            @foreach ($categories as $key => $category)
                <option {{ $sub_category_details->category_id ?? 'selected' }} value="{{ $category->id }}">
    ...
    <div class="col-md-6">
        ...
        <input type="name" name="name" value="{{ $sub_category_details->name }}" class="form-control"
            id="name">
        @error('name')
            <p class="text-danger">{{ $message }}</p>
        @enderror
    ...
    <div class="col-md-3">
        ...
        <input type="number" value="{{ $sub_category_details->display_order }}" name="display_order"
            class="form-control" id="display_order">
        @error('display_order')
            <p class="text-danger">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="d-flex flex-row gap-2">
        <div class="form-check">
            <input class="form-check-input" {{ $sub_category_details->is_active ? 'checked' : '' }} name="is_active"
                type="checkbox">
