...
<x-modal title="Create Users" id="userCreate" size="modal-lg" :centered="true">
    <form action="{{ route('users.store') }}" method="POST" class="row g-3">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="city" class="form-control" id="inputAddress2" placeholder="City name">
            @error('city')
                <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
        <div class="col-md-4">
            ...
            <input type="text" name="district" class="form-control" id="inputAddress3" placeholder="District name">
            @error('district')
                <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
        <div class="col-md-4">
            ...
            <input type="text" name="division" class="form-control" id="inputAddress4" placeholder="Division name">
            @error('division')
                <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
        <div class="col-md-4">
            ...
            <input type="text" name="postal_code" class="form-control" id="inputAddress5" placeholder="Postal Code">
            @error('postal_code')
                <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
...
<script>
    $(function() {
        @if ($errors->any() && url()->previous() === route('users.index'))
            // console.log('{{ url()->previous() }}'); // Check the previous URL in console
            // console.log('{{ route('users.index') }}'); // Check the expected route in console
